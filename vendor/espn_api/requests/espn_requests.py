...
def _get_proxies():
    h = _os.getenv("PROXY_HOST")
    p = _os.getenv("PROXY_PORT")
    u = _os.getenv("PROXY_USER")
    w = _os.getenv("PROXY_PASS")
...
class EspnFantasyRequests(object):
    ...
    def checkRequestStatus(self, status: int, extend: str = "", params: dict = None, headers: dict = None) -> dict:
        ...
        if status == 401:
            ...
            r = requests.get(self.LEAGUE_ENDPOINT + extend, params=params, headers=headers, cookies=self.cookies, proxies=_get_proxies())
    ...
    def league_get(self, params: dict = None, headers: dict = None, extend: str = ''):
        ...
        r = requests.get(endpoint, params=params, headers=headers, cookies=self.cookies, proxies=_get_proxies())
